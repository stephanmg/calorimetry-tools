...
def write_tse(df, gene_symbol, filename, use_selection_of_animals=True):
    ...
    unique_ids = (
        df[["Sample", "Condition", "Weight", "Sex"]]
        .astype(str)
        .agg("_".join, axis=1)
    )
    unique_ids = [
        (*uid.split("_"), index)
        for (index, uid) in enumerate(unique_ids.tolist())
    ]

    ko = list(df.loc[df["Condition"].str.contains("KO", na=False), "Sample"])
    unique_kos = len(list(set(ko)))
    ko = list(set([f"{k}" for k in ko]))
    wt = list(df.loc[df["Condition"].str.contains("WT", na=False), "Sample"])[
        :(unique_kos)
    ]
    ...
    if use_selection_of_animals:
        ...
        filename = f"example_selection_{gene_symbol}.csv"
    ...
    with open(filename, "w") as f:
        ...
        for animal_id in selection_of_animal_ids:
            for _, (animal, genotype, weight, sex, box) in enumerate(
                unique_ids
            ):
                if animal_id == animal:
        ...
        for index, row in df.iterrows():
            ...
            if animal in selection_of_animal_ids:
                ...
                f.write(
                    f"{date};{time};{row['Sample']};{box};{o2};{co2};{weight}\n"
                )
